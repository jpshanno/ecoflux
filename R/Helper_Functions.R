...
#' row (horizontal) panel with a shared legend. If any single plot does not include a legend then a shared legend is excluded, with a message. The legend from the first object in the list is used in the panel, other legends are discarded. 
...
#' @export
#' @rdname ggpanel
ggpanel <- function(plots, orientation = "horizontal") {
  ...
  plotGrobs <- 
    lapply(
      ...
      function(x){
        ggplotGrob(x + 
                     theme(legend.direction = "horizontal"))$grobs})
